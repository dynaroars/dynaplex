...
def closest_point_problem(points, depth, file):
    ...
    point = sorted(point, key=get_key)                 #this line cause dig to fail
    visited_index = []
...
def find_min(start, end, point, heap, visited_index, depth, file):
    with open(file, 'a') as f:
        print("{};2;{}".format(depth, end-start+1), file=f)
...
if __name__ == '__main__':
    for i in range(100):
        ...
        file = "./closest_pair/output-{}".format(len(x))
        closest_point_problem(x, 0, file)
        with open("./closest_pair/traces", 'a') as f:
            print("{};{}".format(len(x), counter), file=f)
        counter = 0
